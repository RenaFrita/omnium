...
export const BboCard = () => {
  const { bids, asks } = useOrderBookStore(
    useShallow((s) => ({ bids: s.bids, asks: s.asks }))
  )
  ...
  return (
    <div className="bg-[#0d1526] border border-slate-800 rounded-md px-3 py-2.5">
      ...
      <div className="flex justify-between">
        <div>
          <div className="text-[22px] font-bold font-mono text-emerald-400 leading-none">
            {fmt(bid)}
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5 font-mono">BID</div>
        </div>
        <div className="text-center">
          <div className="text-[22px] font-bold font-mono text-slate-500 leading-none">
        ...
        <div className="text-right">
          <div className="text-[22px] font-bold font-mono text-rose-400 leading-none">
            {fmt(ask)}
          </div>
      ...
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="font-mono font-semibold text-[12px] text-cyan-400">
          {fmt(mid)}
        </span>
      </div>
      <div className="flex justify-between items-center mb-1">
        <span className="text-[10px] text-slate-500 font-mono">VWAP</span>
        <span className="font-mono font-semibold text-[12px] text-slate-300">
          {vwap ? fmt(vwap) : '\u2014'}
        </span>
      </div>
      {mid && vwap && (
        <div className="flex justify-between items-center">
          ...
          <span
            className={`font-mono font-semibold text-[11px] ${mid > vwap ? 'text-emerald-400' : 'text-rose-400'}`}
          >
